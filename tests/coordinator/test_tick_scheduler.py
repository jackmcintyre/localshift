...
def _task_mock():
    """Return a MagicMock for async_create_task that closes coroutines.

    Prevents 'coroutine was never awaited' RuntimeWarnings when async methods
    are passed to async_create_task but not actually executed in tests.
    """
    return MagicMock(
...
async def test_handle_slow_tick(coordinator):
    ...
    coordinator.hass.async_create_task = _task_mock()
