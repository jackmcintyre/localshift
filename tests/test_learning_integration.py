...
class TestLearningIntegration:
    ...
    def test_weight_updates_based_on_performance(self, components):
        """Test that objective weights update based on historical performance."""
        controller = components["controller"]

        # Create decisions with high export loss rate
        decisions = []
        for i in range(30):
            decision = DecisionRecord(
                ...
                hour_of_day=12,
                is_demand_window=False,
                actual_export_kwh=1.0,  # High export
                outcome_score=0.5,
            )
            decisions.append(decision)
...
class TestLearningSystemEdgeCases:
    ...
    def test_all_bad_decisions(self, components):
        """Test handling all bad decisions (low scores)."""
        controller = components["controller"]

        # Create decisions with all low scores
        decisions = []
        for i in range(30):
            decision = DecisionRecord(
                timestamp=datetime.now() - timedelta(hours=i),
                ...
                hour_of_day=12,
                is_demand_window=False,
                outcome_score=0.2,  # Low score
            )
            decisions.append(decision)

        # Update weights should still work
        controller.update_weights(decisions)
    ...
    def test_rapid_mode_transitions(self, components):
        """Test handling rapid mode transitions in decision history."""
        controller = components["controller"]

        # Create decisions with rapid mode changes
        decisions = []
        modes = [
            BatteryMode.GRID_CHARGING,
        ...
        for i in range(30):
            decision = DecisionRecord(
                ...
                mode_chosen=modes[i % len(modes)],
                previous_mode=modes[(i + 1) % len(modes)],
