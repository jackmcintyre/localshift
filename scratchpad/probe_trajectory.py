...
def traj(n):
    cfg = build_config()
    ctx = (
        patch.object(core_mod, "_constraints_feasible_actions", side_effect=force(n))
        if n
...
b, rb = traj(3)
print(
    f"{'slot':>4} {'time':>6} {'int':>4} | {'DP-choice':>22} | {'forced-0..2':>22} | delta"
)
print(
    f"{'':>4} {'':>6} {'':>4} | {'action':>14}{'soc':>8} | {'action':>14}{'soc':>8} |"
)
for i in range(0, 16):
    ...
    aa = (
        str(getattr(da, "action", ""))
        .split(".")[-1]
        .replace("CHARGE_GRID_", "cg_")
        .lower()
    )
    ab = (
        str(getattr(db, "action", ""))
        .split(".")[-1]
        .replace("CHARGE_GRID_", "cg_")
        .lower()
    )
    print(
        f"{i:>4} {RAW[i][1]:>6} {RAW[i][2]:>3}m | {aa:>14}{sa:>8.2f} | {ab:>14}{sb:>8.2f} | {sb - sa:+6.2f}"
    )
...
for n in (0, 3, 6, 12, 15):
    _, r = traj(n)
    print(
        f"  forced charge slots 0..{n - 1:<2d}  dw_entry={r.dw_entry_soc_pct:6.2f}  "
...
for pen in (0.0, 0.03, 0.10, 1.00, 100.0):
    ...
    print(
        f"  penalty_per_pct={pen:7.2f}  dw_entry={r.dw_entry_soc_pct:6.2f}  "
        f"net_cost={r.projected_net_cost:.4f}"
    )

print("\n--- is the #886 hard floor actually pruning? ---")
for _floor in (None, 95.0):
    cfg = build_config()
    r = DPPlanner().plan(
        _OI(cycle_id="f", initial_soc_pct=INITIAL_SOC, slots=build_slots(), config=cfg)
    )
    print(
        f"  config.hard_target_floor after solve = {cfg.hard_target_floor}  "
