...
def go(label, n):
    cfg = build_config()
    ctx = patch.object(core_mod, "_constraints_feasible_actions",
                       side_effect=force_charge_until(n)) if n else contextlib.nullcontext()
    with ctx:
        r = DPPlanner().plan(OptimizerInputs(cycle_id=label, initial_soc_pct=INITIAL_SOC,
                                             slots=build_slots(), config=cfg))
    traj = {getattr(d, "slot_index", -1): getattr(d, "predicted_soc_pct", None)
            for d in r.decisions}
    print(f"{label:26s} dw_entry={r.dw_entry_soc_pct:6.2f}  shortfall={r.terminal_shortfall_pct:5.2f}  "
