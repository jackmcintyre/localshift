...
def overnight(r):
    return sum(float(getattr(d, "grid_import_kwh", 0.0) or 0.0)
...
def cycles(r):
    ...
    return sum(1 for a, b in zip(acts, acts[1:]) if ("charge_grid" in a) != ("charge_grid" in b))


print("--- today's horizon ---")
for bins in (50, 100):
    cfg = build_config(); cfg.soc_bins = bins
    r = DPPlanner().plan(OptimizerInputs(cycle_id=f"n{bins}", initial_soc_pct=INITIAL_SOC,
                                        slots=build_slots(), config=cfg))
    print(f"  soc_bins={bins:3d}  dw_entry={r.dw_entry_soc_pct:6.2f}  "
...
for idx, hhmm, interval, buy, sell, solar, cons in RAW:
    flat.append(SlotContext(
        slot_index=idx, timestamp_iso=f"2026-08-25T{hhmm}:01+10:00",
        slot_interval_minutes=interval, buy_price=0.17, sell_price=0.08,
        solar_kwh=solar, consumption_kwh=cons,
        is_demand_window_entry=(idx == 15), is_demand_window_slot=(15 <= idx < 27),
        price_source="30min"))
for bins in (50, 100):
    cfg = build_config(); cfg.soc_bins = bins
    r = DPPlanner().plan(OptimizerInputs(cycle_id=f"flat{bins}", initial_soc_pct=INITIAL_SOC,
                                        slots=flat, config=cfg))
    print(f"  soc_bins={bins:3d}  dw_entry={r.dw_entry_soc_pct:6.2f}  "
          f"overnight_grid_charge={overnight(r):.3f} kWh  post-DW action flips={cycles(r)}")
...
for bins in (50, 100):
    cfg = build_config(); cfg.soc_bins = bins
    r = DPPlanner().plan(OptimizerInputs(cycle_id=f"h{bins}", initial_soc_pct=97.0,
                                        slots=build_slots(), config=cfg))
